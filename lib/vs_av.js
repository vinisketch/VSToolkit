define(['exports', 'vs_utils', 'vs_core'], function (exports, vs_utils, vs_core) { 'use strict';

vs_utils = vs_utils && vs_utils.hasOwnProperty('default') ? vs_utils['default'] : vs_utils;
...
Audio.prototype = {
  ...
  play : function ()
  {
    if (this._state === vs_core.Task.STARTED) { return false; }
    if (!this.__audio) { return; }

    this._state = vs_core.Task.STARTED;
    this.__audio.play ();
  },
  ...
  pause : function ()
  {
    if (this._state === vs_core.Task.PAUSED) { return false; }

    if (!this.__audio) { return; }
    this.__audio.pause ();
    this._state = vs_core.Task.PAUSED;
  ...
  stop : function ()
  {
    if (this._state === vs_core.Task.STOPPED) { return false; }
    ...
    { this.__audio.pause (); }
    this._state = vs_core.Task.STOPPED;
  ...
  handleEvent : function (event)
  {
    switch (event.type)
    {
      ...
      case 'ended':
        this._state = vs_core.Task.STOPED;
...
vs_utils.extendClass (Audio, vs_core.EventSource);
  ...
  'src': {
    ...
    set : function (v)
    {
      ...
      if (window.Media)
      {
        function onSuccess ()
        {
          self._state = vs_core.Task.STOPED;

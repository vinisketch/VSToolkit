import vs_utils, { clone, defineClassProperties, defineClassProperty, extendClass, free, importFile, isArray, isFunction as isFunction$1, isNumber, isString as isString$1, isUndefined, requestAnimationFrame, setActiveStyleSheet, underscore } from 'vs_utils';
import vs_gesture, { addPointerListener, removePointerListener } from 'vs_gesture';
...
function doOneEvent (burst, isSynchron) {
  ...
  function doOneHandler (handler) {
    if (handler) try {
      if (isFunction$1 (handler.func_ptr)) {
        // call function
        handler.func_ptr.call (handler.obj, event);
      }
      else if (isString$1 (handler.func_name) &&
               isFunction$1 (handler.obj[handler.func_name]))
      {
        // specific notify method
        handler.obj[handler.func_name] (event);
      }
      else if (isFunction$1 (handler.obj.notify)) {
  ...
  unbind : function (spec, obj, func)
  {
    function unbind (handler_list)
    {
      ...
      while (i < handler_list.length)
      {
        ...
        if (handler.obj === obj)
        {
          if (isString$1 (func) || isFunction$1 (func) )
          {
...
extendClass (EventSource$1, VSObject);
...
extendClass (Model, EventSource$1);
...
function createClass (config)
{
  ...
  if (config) for (var key in config)
  {
    ...
    if (!isFunction$1 (func) || !isFunction$1 (superFunc)) continue;
  ...
  parseData : function (obj)
  {
    ...
    function fillArray (data)
    {
      ...
      for (i = 0; i < data.length; i++)
      {
        ...
        else if (isUndefined (item) || isString$1 (item) ||
...
extendClass (VSArray$1, Model);

/********************************************************************
                  Define class properties
********************************************************************/

defineClassProperties (VSArray$1, {
  "data" : {
    /**
  ...
  "index" : {
    ...
    set : function (v)
    {
      if (isString$1 (v)) v = parseInt (v, 10);
...
extendClass (DataStorage, EventSource$1);
...
extendClass (HTTPRequest$1, EventSource$1);

/********************************************************************
                  Define class properties
********************************************************************/

defineClassProperties (HTTPRequest$1, {
  "url": {
    /**
     * Setter for the url
...
//        var url = self._url + name + '.json';
        ...
        if (ps && ps.length)
        {
          ...
          for (var i = 0; i < ps.length; i ++)
          {
            ...
            if (!isString$1 (value) && !isNumber (value)) continue;
            if (j++) url += '&';
...
export { AjaxJSONP, VSArray$1 as VSArray, createClass, Connector$1 as Connector, DataFlow$1 as Dataflow, DataStorage, DeviceConfiguration, Event, EventSource$1 as EventSource, Fsm, HTTPRequest$1 as HTTPRequest, KEYBOARD$1 as KEYBOARD, LocalStorage, Model, VSObject, RestStorage, scheduleAction, ON_NEXT_FRAME, createId, Task, Task_PAR, Task_SEQ, TaskWait, par, seq };

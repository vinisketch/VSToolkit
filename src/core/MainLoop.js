...
import { requestAnimationFrame, isFunction, isNumber, isString } from 'vs_utils';
...
function doOneEvent (burst, isSynchron) {
  ...
  function doOneHandler (handler) {
    if (handler) try {
      if (isFunction (handler.func_ptr)) {
        // call function
        handler.func_ptr.call (handler.obj, event);
      }
      else if (isString (handler.func_name) &&
               isFunction (handler.obj[handler.func_name]))
      {
        // specific notify method
        handler.obj[handler.func_name] (event);
      }
      else if (isFunction (handler.obj.notify)) {

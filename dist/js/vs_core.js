var vs_core = (function (exports,vs_utils,vs_gesture) {
'use strict';

var vs_utils__default = 'default' in vs_utils ? vs_utils['default'] : vs_utils;
var vs_gesture__default = 'default' in vs_gesture ? vs_gesture['default'] : vs_gesture;
...
DataFlow$1.__nb_propagation = 0;

DataFlow$1.prototype = {
  ...
  connect : function (obj_src, property_out, obj_trg, property_in, func) {
    ...
    if (vs_utils.isString (property_out)) {
      properties_out = [property_out];
    }
    else if (!vs_utils.isArray (property_out)) {
    ...
    if (vs_utils.isString (property_in)) {
      properties_in = [property_in];
    }
    else if (!vs_utils.isArray (property_in)) {
      console.warn ("DataFlow.connect, error");
      return;
    }
    else {
        ...
        for (j = 0; j < edges.length; j++) {
          ...
          for (k = 0; k < properties.length; k++) {
            ...
            if (descriptor.get) properties_temp.push (descriptor.get);
              ...
              }('_' + vs_utils.underscore (property_name))));
            }
          }
...
var _default_df_ = new DataFlow$1();
 ...
 * @private
 */
var _async_events_queue = [];
var _sync_event = null;
var _main_actions_queue = [];
var _tmp_actions_queue = [];
var _is_async_events_propagating = false;
var _is_sync_events_propagating = false;
...
function doOneEvent (burst, isSynchron) {
  ...
  function doOneHandler (handler) {
    if (handler) try {
      if (vs_utils.isFunction (handler.func_ptr)) {
        // call function
        handler.func_ptr.call (handler.obj, event);
      }
      else if (vs_utils.isString (handler.func_name) &&
               vs_utils.isFunction (handler.obj[handler.func_name]))
      {
        // specific notify method
        handler.obj[handler.func_name] (event);
      }
      else if (vs_utils.isFunction (handler.obj.notify)) {
...
if (!window.setImmediate) {
  ...
  window.setImmediate = setImmediate = queueAction.bind (window);
}
else {
...
function scheduleAction (func, delay) {
  if (!vs_utils.isFunction (func)) return;
  if (delay && vs_utils.isNumber (delay)) {
    setTimeout (func, delay);
  }
  else if (delay === ON_NEXT_FRAME) {
    vs_utils.requestAnimationFrame (func);
  }
...
Model.prototype = {
  ...
  clear : function (should_free)
  {
    ...
    _properties_.forEach (function (prop_name) {
      ...
      // free the property
      if (should_free) vs_utils.free (self [_prop_name]);
...
//       }
  ...
  _parse_property : function (prop_name, value)
  {
    var
      _properties_ = this.getModelProperties (),
      desc, _prop_name = '_' + vs_utils.underscore (prop_name), model;

    if ((value && value.data) || vs_utils.isArray (value))
    {
...
vs_utils.extendClass (Model, EventSource);
 ...
 * @param {Object} config the configuration structure
*/
function VSObject$1 (config)
{
  this.constructor = VSObject$1;
  if (vs_utils__default.isString (config)) { this._id = config; }
...
//      delete (this.__config__);
    ...
    if (this.initSkin)
    {
      ...
      VSObject$1.prototype.initSkin = function () {};

      // call the initSkin
      this.initSkin ();

      // remove the fake initSkin
      VSObject$1.prototype.initSkin = undefined;
    }
  ...
  {
    try {
      this.parseData ((json && vs_utils__default.parseJSON (json)) || {});
    }
    catch (e)
    {
...
//           else this ['_' + key] = value; // String
//         }
      this ['_' + vs_utils__default.underscore (key)] = value;
    }
  ...
  _toJSON : function (json)
  {
    ...
    for (var i = 0; i < _properties_.length; i++)
    {
      ...
      value = this ['_' + vs_utils__default.underscore (prop_name)];
  ...
  destructor : function ()
  {
    ...
    delete (VSObject$1._obs [this._id]);
  ...
  unlink : function ()
  {
    ...
    if (this.__model)
    {
      if (this.__model)
      {
        ...
        var props = this.__model.getModelProperties (); l = props.length, config = {};
  ...
  clone : function (config, cloned_map)
  {
    ...
    if (vs_utils__default.isFunction (this.constructor))
    {
    ...
    function _propertyDecl_api1 (prop_name, src, trg)
    {
      ...
      if (!setter && !getter)
      {
        ...
        if (vs_utils__default.isArray (value)) { trg [prop_name] = value.slice (); }
    ...
    function _propertyDecl_api2 (prop_name, src, trg)
    {
      ...
      if (desc && (desc.get || desc.set))
      {
        ...
        if (!desc_clone) {
          vs_utils__default.defineProperty (trg, prop_name, desc);
          if (!trg.__properties__) trg.__properties__ = [];
          trg.__properties__.push (prop_name);
        }
      }
    ...
    for (key in this)
    {
      ...
      // function copy
      if (vs_utils__default.isFunction (this [key]) && !vs_utils__default.isFunction (obj [key]))
      { obj [key] = this [key]; }
  ...
  *************************************************************/
  ...
  {
    vs_utils__default.defineProperty (this, prop_name, descriptor);
...
vs_utils__default.defineClassProperty (VSObject$1, "id", {
  /**
...
{
  if (value && vs_utils__default.isFunction (value.clone)) return value.clone ();
  else return vs_utils__default.clone (value);
}
...
function _propertyCloneValue_api1 (prop_name, src, trg)
{
  var getter = src.__lookupGetter__ (prop_name),
    setter = src.__lookupSetter__ (prop_name),
    setter_clone = trg.__lookupSetter__ (prop_name),
    _prop_name = '_' + vs_utils__default.underscore (prop_name);
...
/** @private */
VSObject$1._obs = {};
VSObject$1.__propertyCloneValue = propertyCloneValue;
 ...
 * @const
 */
var POINTER_START = vs_gesture__default.POINTER_START;
 ...
 * @const
 */
var POINTER_MOVE = vs_gesture__default.POINTER_MOVE;
 ...
 * @const
 */
var POINTER_END = vs_gesture__default.POINTER_END;
 ...
 * @const
 */
var POINTER_CANCEL = vs_gesture__default.POINTER_CANCEL;
 ...
 * @const
 */
var GESTURE_START = vs_gesture__default.GESTURE_START;
 ...
 * @const
 */
var GESTURE_CHANGE = vs_gesture__default.GESTURE_CHANGE;
 ...
 * @const
 */
var GESTURE_END = vs_gesture__default.GESTURE_END;
...
*/
var Event$1 = function (src, type, data)
{
  this.src = src;
  this.srcTarget = src;
  this.type = type;
 ...
 */
function EventSource$1 (config)
{
  this.parent = VSObject$1;
  this.parent (config);
  this.constructor = EventSource$1;
...
EventSource$1.prototype =
{
  ...
  ***************************************************************/
  ...
  destructor : function ()
  {
    ...
    VSObject$1.prototype.destructor.call (this);
  },
  ...
  _clone : function (obj, cloned_map)
  {
    VSObject$1.prototype._clone.call (this, obj, cloned_map);
  ...
  unbind : function (spec, obj, func)
  {
    function unbind (handler_list)
    {
      ...
      while (i < handler_list.length)
      {
        ...
        if (handler.obj === obj)
        {
          if (vs_utils.isString (func) || vs_utils.isFunction (func) )
          {
  ...
  propagate : function (type, data, srcTarget)
  {
    ...
    event = new Event$1 (this, type, data);
  ...
  {
    if (!node) { return; }
    if (!vs_utils.isString (event)) { return; }

    var self = this, func = null, handler = null, binds, key;

    if (typeof (func_s) === "undefined") { func_s = 'notify'; }
    else if (vs_utils.isString (func_s))
    {
      if (!vs_utils.isFunction (this [func_s]))
      {
        console.warn
    ...
    else if (!vs_utils.isFunction (func_s))
    {
    ...
    // set the listener
    vs_gesture.addPointerListener (node, event, handler, false);
  },
  ...
  nodeUnbind : function (node, event, func_s)
  {
    ...
    for (i = 0; i < binds.length;)
    {
      ...
      if (data.n === node)
      {
        vs_gesture.removePointerListener (node, event, data.h, false);
        binds.remove (i);
      }
      else
      {
...
vs_utils.extendClass (EventSource$1, VSObject$1);
...
function createClass (config)
{
  ...
  if (parent.prototype) vs_utils.extendClass (klass, parent);
  ...
  if (config) for (var key in config)
  {
    ...
    if (!vs_utils.isFunction (func) || !vs_utils.isFunction (superFunc)) continue;
...
function _setProperties (klass, properties)
{
  ...
  for (var prop_name in properties)
  {
    ...
    if (vs_utils.isNumber (value))
    {
      export_value = value; desc = {};
      _prop_name = '_' + vs_utils.underscore (prop_name);
      if (export_value & VSObject$1.PROPERTY_IN)
      {
        desc.set = (function (prop_name, _prop_name)
        {
          return function (v)
      ...
      if (export_value & VSObject$1.PROPERTY_OUT)
      {
        desc.get = (function (_prop_name)
        {
          return function ()
    ...
    // 2) export path
    else if (vs_utils.isString (value))
    {
  ...
  vs_utils.defineClassProperties (klass, descriptions);
}
 ...
 *********************************************************/
  ...
  send : function ()
  {
    ...
    var
      script = vs_utils.importFile (script_src, null, null, 'js'),
 ...
 * @param {Object} config the configuration structure
*/
function VSArray$1 (config)
{
  this.parent = Model;
  this.parent (config);
  this.constructor = VSArray$1;
}
  ...
  parseData : function (obj)
  {
    ...
    function fillArray (data)
    {
      ...
      for (i = 0; i < data.length; i++)
      {
        ...
        else if (vs_utils.isArray (item)) {
          _model = new VSArray$1 ().init ();
        }
        
        else if (vs_utils.isUndefined (item) || vs_utils.isString (item) ||
          vs_utils.isNumber (item) || typeof item == "boolean" ||
...
vs_utils.extendClass (VSArray$1, Model);

/********************************************************************
                  Define class properties
********************************************************************/

vs_utils.defineClassProperties (VSArray$1, {
  "data" : {
    /**
    ...
    set : function (v)
    {
      ...
      else if (v instanceof VSArray$1)
      {
  ...
  "modelClass" : {
    ...
    set : function (v)
    {
      if (!(vs_utils.isFunction (v))) return;
  ...
  "index" : {
    ...
    set : function (v)
    {
      if (vs_utils.isString (v)) v = parseInt (v, 10);
 ...
 *        from
 */
var Connector$1 = function (object, property_name) {
 ...
 *        to
 */
Connector$1.prototype.to = function (object, property_name, func) {
 ...
 *        from
 */
Connector$1.prototype.connect = function (property_name) {
  var object = this._previous_object || this._base_object;
  
  return new Connector$1 (object, property_name);
};
...
{
  this.parent = EventSource$1;
  this.parent (config);
  this.constructor = DataStorage;
...
vs_utils.extendClass (DataStorage, EventSource$1);
...
//        this.versionSearchString = data[i].versionSearch || data[i].identity;
  ...
  setDeviceId : function (did)
  {
    ...
    if (!vs_utils.isString (did)) return; 
  ...
  setOrientation : function (orientation, force)
  {
    ...
    for (id in VSObject$1._obs)
    {
      comp = VSObject$1._obs [id];
      if (!comp) { continue; }
  ...
  {
    vs_utils.setActiveStyleSheet (pid);
  },
...
vs_utils.extendClass (Fsm, EventSource);

/*****************************************************************
 *     Properties declaration
 ****************************************************************/

vs_utils.defineClassProperty (Fsm, "initialState", {
  /**
 ...
 * @param {Object} config the configuration structure
 */
var HTTPRequest$1 = function (config)
{
  this.parent = EventSource$1;
  this.parent (config);
  this.constructor = HTTPRequest$1;

  this._headers = {};
  this.__xhrs = [];
};

HTTPRequest$1.prototype = {
 ...
 *********************************************************/
  ...
  destructor : function ()
  {
    ...
    EventSource$1.prototype.destructor.call (this);
  },
...
vs_utils.extendClass (HTTPRequest$1, EventSource$1);

/********************************************************************
                  Define class properties
********************************************************************/

vs_utils.defineClassProperties (HTTPRequest$1, {
  "url": {
    /**
     * Setter for the url
 ...
 *  @const
 */
var KEYBOARD$1 = new EventSource$1 ('__KEYBOARD__');

/**
 * @private
 */
KEYBOARD$1._handler_set_down = false;

/**
 * @private
 */
KEYBOARD$1._handler_set_up = false;
...
{
  var handler = EventSource$1.prototype.bind.call (this, keyCode, obj, func),
    self = this;
  if (prevent) { handler.prevent = true; }
  if (keyCode > KEYBOARD$1.KEY_UP)
  {
    if (!this._handler_set_up)
    {
        ...
        ("keyup", function (event)
      {
        ...
        self.propagate (event.keyCode + KEYBOARD$1.KEY_UP, event);
      }, false);
    ...
    if (!this._handler_set_down)
    {
        ...
        ("keydown", function (event)
      {
        if ((event.ctrlKey || event.metaKey) &&
            !event.shiftKey && event.keyCode === KEYBOARD$1.Z)
        {
          self.propagate (KEYBOARD$1.UNDO);
          event.preventDefault ();
        }
        else if ((event.ctrlKey || event.metaKey) &&
                  event.shiftKey && event.keyCode === KEYBOARD$1.Z)
        {
          self.propagate (KEYBOARD$1.REDO);
          event.preventDefault ();
        }
        else if ((event.ctrlKey || event.metaKey) && !event.shiftKey && event.keyCode === KEYBOARD$1.S)
        {
          self.propagate (KEYBOARD$1.SAVE);
          event.preventDefault ();
        }
        else if ((event.ctrlKey || event.metaKey) && !event.shiftKey && event.keyCode)
        {
          self.managePrevent (KEYBOARD$1.META + event.keyCode, event);
          self.propagate (KEYBOARD$1.META + event.keyCode);
          //event.preventDefault ();
        }
        else
        {
 ...
 *  @private
 */
KEYBOARD$1.managePrevent = function (type, event)
{
...
KEYBOARD$1.KEY_UP = 1000; 
KEYBOARD$1.ESC = 27;
KEYBOARD$1.ENTER = 13;
KEYBOARD$1.SPACE = 32;
KEYBOARD$1.BACKSPACE = 8;
KEYBOARD$1.SHIFT = 16;
KEYBOARD$1.CTRL = 17;
KEYBOARD$1.ALT = 18;
KEYBOARD$1.NUMLOCK = 144;

KEYBOARD$1.LEFT_ARROW = 37;
KEYBOARD$1.UP_ARROW = 38;
KEYBOARD$1.RIGHT_ARROW = 39;
KEYBOARD$1.DOWN_ARROW = 40;
...
KEYBOARD$1.META = 2000;
KEYBOARD$1.ANY_MASK = 3000;
...
vs_utils.extendClass (LocalStorage, DataStorage);
...
//        var url = self._url + name + '.json';
        ...
        if (ps && ps.length)
        {
          ...
          {
            var prop_name = ps[i], value = model ['_' + vs_utils.underscore (prop_name)];
            if (prop_name === "id" || prop_name === 'modelClass') continue
            if (!vs_utils.isString (value) && !vs_utils.isNumber (value)) continue;
            if (j++) url += '&';
...
vs_utils.extendClass (RestStorage, DataStorage);

/********************************************************************
                  Define class properties
********************************************************************/

vs_utils.defineClassProperties (RestStorage, {
  "url": {
    /**
     * Setter for the url
...
//
// };
//extendClass (Scheduler, EventSource);
//
...
********************************************************************/
  ...
  _clone : function (obj, cloned_map)
  {
    VSObject$1.prototype.destructor._clone.call (this, obj, cloned_map);
...
vs_utils.extendClass (Task, VSObject$1);

/********************************************************************
                  Define class properties
********************************************************************/

vs_utils.defineClassProperty (Task, "state", {
...
********************************************************************/
  ...
  setTasks : function (tasksAndParams)
  {
    ...
    for (i = 0; i < tasksAndParams.length; i ++)
    {
      ...
      if (vs_utils.isArray (taskAndparam))
      {
...
vs_utils.extendClass (Task_PAR, Task);
...
vs_utils.extendClass (Task_SEQ, Task);
...
vs_utils.extendClass (TaskWait, Task);

vs_utils.defineClassProperty (TaskWait, "state", {
...
exports.VSArray = VSArray$1;
exports.createClass = createClass;
exports.Connector = Connector$1;
exports.Dataflow = DataFlow$1;
exports.DataStorage = DataStorage;
exports.DeviceConfiguration = DeviceConfiguration;
exports.Event = Event$1;
exports.EventSource = EventSource$1;
exports.Fsm = Fsm;
exports.HTTPRequest = HTTPRequest$1;
exports.KEYBOARD = KEYBOARD$1;
exports.LocalStorage = LocalStorage;
exports.Model = Model;
exports.VSObject = VSObject$1;
exports.RestStorage = RestStorage;
exports.scheduleAction = scheduleAction;
exports.ON_NEXT_FRAME = ON_NEXT_FRAME;
exports.createId = createId;
exports.Task = Task;
exports.Task_PAR = Task_PAR;
exports.Task_SEQ = Task_SEQ;
exports.TaskWait = TaskWait;
exports.par = par;
exports.seq = seq;
